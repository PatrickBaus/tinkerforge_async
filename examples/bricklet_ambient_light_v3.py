...
"""
An example to demonstrate most of the capabilities of the Tinkerforge Ambient Light Bricklet 3.0.
"""
import asyncio
import warnings
...
async def process_callbacks(device: BrickletAmbientLightV3) -> None:
...
async def run_example(bricklet: BrickletAmbientLightV3) -> None:
    ...
    try:
        ...
        await bricklet.set_illuminance_callback_configuration(
            period=1000, value_has_to_change=False, option=bricklet.ThresholdOption.OFF, minimum=0, maximum=0
        )
    ...
    try:
        ...
        async with IPConnectionAsync(host="127.0.0.1", port=4223) as connection:
            ...
            async for enumeration_type, device in connection.read_enumeration():  # pylint: disable=unused-variable
                if isinstance(device, BrickletAmbientLightV3):
